            ...
            for var in focus_vars:
                if var in i[0].columns:
                    if option == 2:
                        ...
                        ax.plot(pd.to_datetime(j[0]['Fecha']), j[0][f"{var}_mean"], label='Real', color=(105/255, 255/255, 231/255))
                        ax.set_title(f'Parcela {i[1]} - Índice {var.upper()}')
                        ax.set_ylabel(f'{var.upper()}')
                        ax.set_xlabel('Fecha')
                        ax.legend()
                        plt.tight_layout()
                        export_path = PurePath(rf"C:\Users\Isai\Documents\Tesis\code\tesis_img\series_tiempo_real_modelo\zafra{zafra}\{var}")
                        if not os.path.exists(export_path):
                            os.makedirs(export_path)
                        plt.savefig(PurePath(export_path, f"parcela_{i[1]}_{var}.png"), dpi=100)
                    elif option == 1:
                        fig, ax = plt.subplots(figsize=(10, 5))
                        ax.plot(pd.to_datetime(j[0]['Fecha']), j[0][f"{var}_mean"], label='Real', color=(150/255, 0, 24/255))
                        ax.set_title(f'Parcela {i[1]} - Índice {var.upper()}')
                        ax.set_ylabel(f'{var.upper()}')
                        ax.set_xlabel('Fecha')
                        ax.legend()
                        plt.tight_layout()
                        export_path = PurePath(rf"C:\Users\Isai\Documents\Tesis\code\tesis_img\series_tiempo_real\zafra{zafra}\{var}")
                        if not os.path.exists(export_path):
                            os.makedirs(export_path)
                        plt.savefig(PurePath(export_path, f"parcela_{i[1]}_{var}.png"), dpi=100)